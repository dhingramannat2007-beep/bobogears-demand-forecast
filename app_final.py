...
if sales_files and inventory_file and limit_files:
    ...
    # We bring in each warehouse's "Sold Last 30d" + trend from the inventory file.
    merged = lim.merge(
        inv[["Warehouse", "UPC", "Sold Last 30d", "Trend Multiplier", "Inv Stock (fallback)"]],
        on=["Warehouse", "UPC"], how="left",
    )
